...
public class UserController {
    ...
    public void get(RoutingContext ctx) {
        ...
        userService.get(userId).onSuccess(user -> ctx.response().setStatusCode(200).end(user.encode()))
    ...
    public void update(RoutingContext ctx) {
        ...
        userService.update(userId, updateData).onSuccess(updatedUser -> ctx.response()
                .setStatusCode(200)
                        .end(new JsonObject().put("userInfo", updatedUser).encode()))
                .onFailure(err -> ctx.response().setStatusCode(400)
                        .end(new JsonObject()
                                .put(MyError.KEY, MyError.BAD_REQUEST).encode()));
    }
    ...
    public void getMembers(RoutingContext ctx) {
        userService.getMembers()
                ...
                .onFailure(err -> ctx.response().setStatusCode(404)
                        .end(new JsonObject()
                                .put(MyError.KEY, "Members not found").encode()));
    }
